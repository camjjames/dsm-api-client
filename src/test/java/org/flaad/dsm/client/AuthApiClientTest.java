...
class AuthApiClientTest extends DsmApiClient {
    ...
    @Test
    @Order(2)
    void whenGetAuthTokenWithDeviceIdApi_thenAuthTokenIsReturned() throws IOException {
        setupWireMock_DsmAuthEndpoint("stubs/api-auth-get-success.json");

        AuthRequest authRequest = AuthRequest.builder().account("user").passwd("pwd").
                deviceId("Uu7ByHpmH0TaAdiRJFH2kGjc8lBVdNezfdfdFIAPUsolvvy9f80ndshAa5y1n6AoDFPz0Fjt5NWPzkVAjrS5Fw").
                build();
        DsmApiResponse<AuthSessionToken> response = authClient.login(authRequest);
